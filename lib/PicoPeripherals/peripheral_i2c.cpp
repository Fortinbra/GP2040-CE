...
{
    printf("block: %u, sda: %u, scl: %u, speed: %u\n", block, sda, scl, speed);
...
{
    if ((_SDA + 2 * i2c_hw_index(_I2C)) % 4 != 0)
        return;
    if ((_SCL + 3 + 2 * i2c_hw_index(_I2C)) % 4 != 0)
        return;
...
{
    printf("write(address: %u, data: %p, len: %u, isBlock: %d)\n", address, data, len, isBlock);
